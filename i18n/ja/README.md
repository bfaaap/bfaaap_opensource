...
- **IQ‑FORTIQ‑M42BLS‑100** モーター *（参照=v039B；EOL → 後継は DRV8825互換インターフェースの閉ループ・ステッパ）*。
  **2GTベルト → T10リードスクリュー → 押し棒** をアルミ押出フレーム上で駆動
...
特に**障害のある方（や必要とする方）がより包摂的に社会参加できる製品・サービス（有償・商用を含む）には、方針として
