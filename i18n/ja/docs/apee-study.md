...
APEE は人を対象とした試験で、成人・子ども（7〜11歳）・障害のある方が参加しました。参加は**任意**で、
**全参加者から書面によるインフォームド・コンセントを取得**しています。

- **成人**は本人が書面で同意しました。
- **子ども（7〜11歳）**は、保護者が（試験を手伝ったピアノ講師を通じて）書面同意を確認したうえで、
  本人が同意書に署名しました。
- **障害のある参加者**は、保護者の書面同意のうえで参加し、保護者が付き添い・送り迎えを行いました。

いつでも中断・中止でき、課題（短いピアノの一節の演奏）のリスクは最小限でした。著者は倫理審査委員会
（IRB）を持つ機関に所属しない独立の実務家のため、正式な IRB 承認番号はありません。それでも本試験は
[ACM の人対象研究に関する方針](https://www.acm.org/publications/policies/research-involving-human-participants-and-subjects)
の倫理原則（危害の最小化、プライバシーと自己決定権の保護）に従って実施しました。ここで示すデータは
すべて**匿名化**（参加者は No.1〜15）されており、個人を特定できる情報は含みません。
