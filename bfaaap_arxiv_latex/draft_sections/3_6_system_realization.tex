...
multiplier $10$--$50$, full action within $+2$--$10^{\circ}$, and a user-tunable
response speed) physically live in the stack.
...
awake during play so the hands-free session is uninterrupted.
...
adjustable in advance to each user's preference, which is what lets users with a
restricted range of motion (small offset, large multiplier) operate the system.
...
giving the performer eyes-free confirmation that matches the device-side
indicator (Section~\ref{sec:switch}).
...
acoustic piano. First, an \emph{airback}: at power-up a pump (driven through a
